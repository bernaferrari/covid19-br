...
export default function define(runtime, observer) {
  ...
  main.variable(observer()).define(["md"], function (md) {
    return md`
# Brazil Contour COVID-19 map

_Fonte: [covid19-br](https://brasil.io/api/dataset/covid19)_
    `;
  });
  main
    ...
    .define(
      ...
      function (d3, DOM, w, h, statesOuter, path, contours, thresholdColorScale, statesInner) {
        const svg = d3
          .create("svg")
          // .attr("width", w)
          // .attr("height", h)
      ...
      function (rawPoints, recentData, between) {
        return rawPoints.map((d) => {
          const results = recentData.filter(
            (dd) =>
              between(dd["longitude"], d[0] - 2, d[0] + 2) &&
              between(dd["latitude"], d[1] - 2, d[1] + 2),
          );
  ...
  main
    ...
    .define(
      "get_point_data",
      ["d3", "provinces", "voronoiLookup"],
      function (d3, provinces, voronoiLookup) {
        return (d, i) => {
          let data = null;
  ...
  main
    ...
    .define("statesOuter", ["topojson", "brasil"], function (topojson, brasil) {
      return topojson.mesh(brasil, brasil.objects.Brasil, (a, b) => a === b);
    });
  main
  ...
  main
    ...
    .define("recentData", ["data_covid", "data_city"], function (data_covid, data_city) {
  ...
  main
    ...
    .define("data_covid", ["d3", "FileAttachment"], async function (d3, FileAttachment) {
      return await d3.csv("/data/br_heatmap.csv", (d) => {
  ...
  main
    ...
    .define("projection", ["d3", "w", "h", "statesOuter"], function (d3, w, h, statesOuter) {
