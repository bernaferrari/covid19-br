...
export default function define(runtime, observer) {
  ...
  main.variable(observer()).define(["md", "x"], function (md, x) {
    return md`# Crescimento do COVID-19 no Brasil
...
Fonte: [covid19-br](https://brasil.io/api/dataset/covid19)`;
  ...
  main
    .variable(observer("viewof indicator"))
    .define("viewof indicator", ["radio"], function (radio) {
      return radio({
        options: [
  ...
  main
    .variable(observer("indicator"))
    .define("indicator", ["Generators", "viewof indicator"], (G, _) => G.input(_));
  main
    .variable(observer("chart"))
    .define(
      "chart",
      ["d3", "w", "height", "colors", "x", "xAxis", "yAxis", "grid", "data", "line", "hover"],
      function (d3, w, height, colors, x, xAxis, yAxis, grid, data, line, hover) {
        const svg = d3
          .create("svg")
        ...
        bar
          ...
          .attr("x", -28)
          .attr("y", 14)
          .style("fill", (d, i) => colors[array.indexOf(d)]);
  ...
  main.variable(observer("data_from_csv")).define("data_from_csv", ["d3"], async function (d3) {
    return await d3.csv("/data/states_alldays.csv");
  });
  main
    .variable(observer("data"))
    .define("data", ["data_from_csv", "d3", "indicator"], function (data_from_csv, d3, indicator) {
      const data = data_from_csv;
      const parseDate = d3.utcParse("%Y-%m-%d");
      const dates = Array.from(d3array.group(data, (d) => d.date).keys(), parseDate).sort(
        d3.ascending,
      );
  ...
  main
    .variable(observer("colorForState"))
    .define(
      "colorForState",
      ["retrieveStateRegion", "colors"],
      function (retrieveStateRegion, colors) {
        return function colorForState(state) {
  ...
  main
    ...
    .define(
      "hover",
      ["colorForState", "x", "data", "y", "indicator", "margin", "d3"],
      function (colorForState, x, data, y, indicator, margin, d3) {
        return function hover(svg, { path, focus }) {
          if ("ontouchstart" in document)
            svg
              .style("-webkit-tap-highlight-color", "transparent")
              .on("touchstart", moved)
              .on("touchend", left);
          else svg.on("mousemove mouseenter", moved).on("mouseleave", left);
  ...
  main.variable(observer("height")).define("height", function () {
    return document.getElementById("externalDivForDaily").clientHeight;
  });
  main.variable(observer("w")).define("w", ["width"], function (width) {
    return document.getElementById("externalDivForDaily").clientWidth;
  });
  main.variable(observer("margin")).define("margin", function () {
    return { top: 40, right: 40, bottom: 40, left: 60 };
  });
  main
    .variable(observer("x"))
    .define("x", ["d3", "data", "margin", "w"], function (d3, data, margin, w) {
      return d3
        .scaleUtc()
  ...
  main
    .variable(observer("y"))
    .define("y", ["d3", "data", "height", "margin"], function (d3, data, height, margin) {
      return d3
        .scaleSqrt()
  ...
  main
    ...
    .define("xAxis", ["height", "margin", "d3", "x", "w"], function (height, margin, d3, x, w) {
      return (g) =>
  ...
  main
    ...
    .define("yAxis", ["margin", "d3", "y", "data"], function (margin, d3, y, data) {
      return (g) =>
        g
  ...
  main
    ...
    .define("grid", ["x", "margin", "height", "y", "w"], function (x, margin, height, y, w) {
      return (g) =>
        g
