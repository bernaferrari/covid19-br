...
export default function define(runtime, observer) {
  ...
  main.variable(observer()).define(["md", "dates"], function (md, dates) {
    return md`# Brazil Coronavirus Daily Cases Map (COVID-19)
...
*Fonte: [covid19-br](https://brasil.io/api/dataset/covid19)*`;
  ...
  main
    ...
    .define("confirmed_or_deaths", ["Generators", "viewof confirmed_or_deaths"], (G, _) =>
      G.input(_),
    );
  ...
  main
    ...
    .define("viewof day", ["Scrubber", "dates", "delay"], function (Scrubber, dates, delay) {
  ...
  main
    ...
    .define(
      ...
      function (
        ...
        const svg = d3.create("svg").attr("viewBox", [0, 0, w, h]).attr("class", "italy");
        ...
        if (scale === "bolhas") {
          ...
          const legend = svg
            ...
            .attr("class", "legend")
            .attr("fill", "#777")
            .attr("transform", `translate(${w > breakpoint ? [10, h - 25] : [10, h - 25]})`);
  ...
  main.variable(observer("style")).define("style", ["html"], function (html) {
    ...
    return html`<style>
      ...
      .place-label {
        text-shadow:
          ${c} 1px 0px 0px,
          ${c} 0.540302px 0.841471px 0px,
          ${c} -0.416147px 0.909297px 0px,
          ${c} -0.989992px 0.14112px 0px,
          ${c} -0.653644px -0.756802px 0px,
          ${c} 0.283662px -0.958924px 0px,
  ...
  main.variable(observer("legendRadii")).define("legendRadii", ["maxCases"], function (maxCases) {
    return [maxCases / 8, maxCases / 4, maxCases / 2, maxCases].map((d) => Math.round(d));
  });
  main
    .variable(observer("currentData"))
    .define("currentData", ["data", "index"], function (data, index) {
      return data[Object.keys(data)[Object.keys(data).length - 1 - index]];
    });
  ...
  main
    ...
    .define("indexSetter", ["mutable index", "dates", "day"], function ($0, dates, day) {
      $0.value = dates.indexOf(day);
    });
  main
    .variable(observer("radius"))
    .define("radius", ["d3", "maxCases", "maxRadius"], function (d3, maxCases, maxRadius) {
  ...
  main
    ...
    .define(
      "maxCases",
      ["data", "d3", "confirmed_or_deaths"],
      function (data, d3, confirmed_or_deaths) {
  ...
  main
    ...
    .define("data", ["d3", "data_city"], async function (d3, data_city) {
  ...
  main
    ...
    .define("dates", ["data", "parseDate"], function (data, parseDate) {
  ...
  main
    ...
    .define(
      "dateExtent",
      ["d3", "data_city_covid", "parseDate"],
      function (d3, data_city_covid, parseDate) {
        return d3.extent(data_city_covid, (r) => parseDate(r.date));
      },
    );
  main
    .variable(observer("provinces"))
    .define("provinces", ["topojson", "brasil"], function (topojson, brasil) {
      return topojson.feature(brasil, brasil.objects.Brasil);
    });
  ...
  main
    ...
    .define(
      "projection",
      ["d3", "showSubtitles", "w", "h", "provinces"],
      function (d3, showSubtitles, w, h, provinces) {
        return d3.geoMercator().fitExtent(
          [
            [20, 0],
            [showSubtitles ? w - 60 : w - 20, h],
          ],
          provinces,
        );
      },
