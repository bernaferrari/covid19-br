...
*Fonte: [covid19-br](https://brasil.io/api/dataset/covid19)*`;
  ...
  main
    ...
    .define(
      ...
      function (d3, confirmed_or_deaths, maxCases) {
        return d3
          .scaleSequentialSqrt(
            confirmed_or_deaths === "c" ? d3.interpolateYlGnBu : d3.interpolateYlOrRd,
          )
          .domain([0, maxCases]);
      },
    );
  ...
  main
    ...
    .define("maxLegend", ["maxCases", "magnitude"], function (maxCases, magnitude) {
      return Math.round(maxCases / magnitude) * magnitude;
    });
  main
    .variable(observer("magnitude"))
    .define("magnitude", ["toMagnitude", "maxCases"], function (toMagnitude, maxCases) {
  ...
  main.variable(observer("data")).define("data", ["d3"], async function (d3) {
    const rows = await d3.csv("/data/pr_ndays.csv", (d) => {
      d["z"] = +d["z"];
      d["c"] = +d["c"];
      d["d"] = +d["d"];
      return d;
  ...
  main
    ...
    .define("topCities", ["data", "confirmed_or_deaths"], function (data, confirmed_or_deaths) {
  ...
  main
    ...
    .define(
      "places",
      ["topCities", "estado", "data_city"],
      function (topCities, estado, data_city) {
